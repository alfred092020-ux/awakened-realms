...
describe('Global 3.0.24 reverse-engineering completeness ledger', () => {
  ...
  it('marks only external evidence ceilings as remaining', () => {
    ...
    expect(LOGRES_GLOBAL_3024_EVIDENCE_CEILINGS)
      .toContain('The Global GmCl transport chain is recovered as SplitToContract -> Snappy Compressor -> Packetize; linked Blowfish code has no proven active GmCl checkpoint.')
    expect(LOGRES_GLOBAL_3024_EVIDENCE_CEILINGS.join(' '))
      .not.toContain('concrete encryption algorithm/policy are not fully recovered')
