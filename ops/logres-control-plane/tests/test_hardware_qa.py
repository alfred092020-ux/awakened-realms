...
def battle_state(*, show_demo=False, accepted=0, threshold=3):
    return {
...
class HardwareQaTests(unittest.TestCase):
    def test_success_path_uses_production_registry_contract(self):
        plan = qa.playable_battle_plan(battle_state())
        self.assertEqual(
            {"x": 489.6, "y": 563.2, "threshold": 3, "accepted": 0},
            plan,
        )

        progress = battle_state(accepted=1)
    ...
    def test_runner_no_longer_hardcodes_current_runtime_block(self):
        text = SCRIPT.read_text()
        self.assertNotIn("BLOCKED_CURRENT_RUNTIME_PATH", text)
        self.assertNotIn("BLOCKED_HARNESS_ONLY", text)
