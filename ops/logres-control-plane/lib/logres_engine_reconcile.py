...
COPILOT_LIVE={"ACTIVE","VERIFYING","QUEUED"}
ROUTE_LIVE={"ACTIVE","VERIFYING","QUEUED","ROUTED"}
...
def plan(conn):
    actions=[]
    for r in conn.execute("""select c.id,c.task_id,c.state,t.status,c.branch,c.candidate_sha
      from copilot_jobs c left join tasks t on t.id=c.task_id
      where c.state in ('ACTIVE','VERIFYING','QUEUED')"""):
    ...
    for r in conn.execute("""select r.id,r.task_id,r.state,t.status,r.external_ref,r.last_error
      from route_jobs r left join tasks t on t.id=r.task_id
      where r.state in ('ACTIVE','VERIFYING','QUEUED','ROUTED')"""):
        task_status=r[3] or "MISSING"
        if task_status in TERMINAL_TASK or task_status=="MISSING":
...
def apply(conn):
    ...
    for a in actions:
        if a["engine"]=="copilot":
            ...
            conn.execute("update route_jobs set state='SUPERSEDED',updated_at=? where id=? and state=?",
                         (stamp,a["id"],a["from_state"]))
        elif a["engine"]=="swarm":
